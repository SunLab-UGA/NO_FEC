...
def find_frame_pairs(tx_time, rx_time, max_delta=1.0):
    '''this function pairs the closest rx to tx times, the remaining tx times are dropped frames'''
    # track the delta between tx and rx pairs
    delta_times = []
    valid_frame_indexes = []
    dropped_frame_indexes = []
    # loop through the tx times
    max_delta = timedelta(seconds=max_delta)  # Convert max_delta to timedelta object

    for ii, tx in enumerate(tx_time):
        # find the closest rx time
        closest_rx = min(rx_time, key=lambda x: abs(x - tx))
        # check if the delta is within the max delta
        if abs(closest_rx - tx) > max_delta and ii != 0: # ignore the first frame (this is not a great solution, but the first frame takes longer for some reason...)
            print(f"WARNING: Dropped frame detected (index: {ii}) delta: {abs(closest_rx - tx)} s @ time: {tx}")
            dropped_frame_indexes.append(ii)
        else:
            # add the delta to the list
            delta_times.append(closest_rx - tx)
            valid_frame_indexes.append(ii)


    # construct the non-dropped tx times
    valid_tx_times = [tx_time[i] for i in valid_frame_indexes]
    return valid_tx_times, valid_frame_indexes, delta_times # tx_time is now the dropped frames, delta_times is the time between paired tx and rx
...
# load the file
rx_data_time, rx_data = load_file(os.path.join(newest_dir, rx_data_file), type=np.uint8)
...
# load the file
tx_data_time, tx_data = load_file(os.path.join(newest_dir, tx_data_file), type=np.uint8)

# print the len of the data
print(f"tx_data len: {len(tx_data)}")
...
if len(rx_data) == len(tx_data):
    ...
    print(f"RX data is longer by {len(rx_data) - len(tx_data)} frames!?")
else:
    print(f"TX data is longer by {len(tx_data) - len(rx_data)} frames")
    # find the dropped frames
    max_time_delta = 1 # seconds
    print(f"finding valid frames pairs with max_time_delta of {max_time_delta} s")
    valid_tx_times, valid_tx_indexes, delta_times = find_frame_pairs(tx_data_time, rx_data_time, max_delta=max_time_delta) # by time
    print(f"num valid frames found: {len(valid_tx_times)}")
    # print the valid_tx_indexes
    print(f"valid_tx_times: {valid_tx_indexes}")
...
# remove the dropped frames from the tx data
tx_data = [tx_data[i] for i in valid_tx_indexes]

# check if the data is the same length
if len(rx_data) == len(tx_data):
    print("Data frames are now the same length")
else:
    print("WARNING: Data frames are not the same length after removing dropped frames!?")
    print("Removing last tx frame...",end='')
    tx_data = tx_data[:-1]
    if len(rx_data) == len(tx_data):
        print("Data frames are now the same length")
    else:
        print("WARNING: Data frames are not the same length after removing last tx frame!?")
...
# print the first frame tx/rx time
print("first frame tx/rx time:")
print(f"tx_data_time[0]: {tx_data_time[0]}")
print(f"rx_data_time[0]: {rx_data_time[0]}")

# print the first data frame tx/rx
print("first frame tx/rx data:")
print(f"tx_data[0]: {tx_data[0]}")
print(f"rx_data[0]: {rx_data[0]}")
# print the last data frame tx/rx
print("last frame tx/rx data:")
print(f"tx_data[-1]: {tx_data[-1]}")
print(f"rx_data[-1]: {rx_data[-1]}")
...
full_bit_tot = 0
frame_ber = []
for t,r in zip(tx_data, rx_data):
    # calculate the BER
